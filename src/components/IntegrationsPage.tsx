...
function StatusBadge({ status }: { status: IntegrationStatus }) {
  ...
  return (
    <span className="text-[10px] font-mono text-[#444444]">Available</span>
  );
}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#080808]">
      ...
      <div className="border-b border-[#1e1e1e] px-8 py-6 shrink-0">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#555555] mt-1">Kết nối dịch vụ bên ngoài với TerKix workspace.</p>
          </div>
        ...
        {connectedList.length > 0 && (
          <div className="flex items-center gap-3 mt-5 flex-wrap">
            <span className="text-xs font-mono text-[#444444]">{connectedList.length} connected:</span>
            {connectedList.map(i => (
              <span key={i.id} className="flex items-center gap-1.5 text-xs font-mono text-[#4B7FFF] border border-[#4B7FFF]/20 bg-[#4B7FFF]/5 px-2.5 py-1">
                <span className="w-1 h-1 bg-[#4B7FFF]" />
                {i.name}
              </span>
            ))}
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Category sidebar */}
        <div className="w-48 shrink-0 border-r border-[#1e1e1e] py-4 overflow-y-auto">
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              ...
              >
                <Icon size={12} className={category === cat ? "text-[#4B7FFF]" : ""} />
                {cat}
              </button>
            );
          })}
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#1e1e1e]">
            {filtered.map(integration => {
              ...
              return (
                <div
                  key={integration.id}
                  className="bg-[#080808] p-5 hover:bg-[#0a0a0a] transition-colors group"
                >
                  {/* Header */}
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-[#0f0f0f] border border-[#1e1e1e] flex items-center justify-center font-mono font-bold text-xs text-[#888888]">
                        {integration.icon}
                      </div>
                      <div>
                        <div className="text-sm font-semibold text-white">{integration.name}</div>
                        <div className="text-[10px] font-mono text-[#444444] mt-0.5">{integration.category}</div>
                      </div>
                    </div>
                    <StatusBadge status={isConnected ? "connected" : integration.status === "error" ? "error" : "available"} />
                  </div>

                  {/* Description */}
                  <p className="text-xs text-[#555555] leading-relaxed mb-4">{integration.desc}</p>

                  {/* Tags */}
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {integration.tags.map(t => (
                      <span key={t} className="text-[10px] font-mono border border-[#1e1e1e] text-[#333333] px-2 py-0.5">{t}</span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-1">
                    {isConnected ? (
                      <>
                        <button className="flex items-center gap-1.5 text-xs font-mono text-[#444444] hover:text-[#888888] transition cursor-pointer border border-[#1e1e1e] px-3 py-1.5">
                          <RefreshCw size={11} /> Sync
                        </button>
                        <button
                          onClick={() => setConnected(prev => { const s = new Set(prev); s.delete(integration.id); return s; })}
                          className="flex items-center gap-1.5 text-xs font-mono text-[#444444] hover:text-red-400 transition cursor-pointer border border-[#1e1e1e] hover:border-red-500/30 px-3 py-1.5 ml-auto"
                        >
                          Disconnect
                        </button>
                      </>
                      ...
                      <button
                        onClick={() => setConnected(prev => new Set([...prev, integration.id]))}
                        className="flex items-center gap-1.5 text-xs font-mono text-red-400 hover:text-white border border-red-500/30 hover:border-red-500/60 px-3 py-1.5 transition cursor-pointer"
                      >
                      ...
                      <button
                        onClick={() => setConnected(prev => new Set([...prev, integration.id]))}
                        className="flex items-center gap-1.5 text-xs font-mono text-white bg-[#4B7FFF] hover:bg-[#6B9FFF] px-3 py-1.5 transition cursor-pointer"
                      >
          ...
          {filtered.length === 0 && (
            <div className="py-24 text-center border border-[#1e1e1e] flex flex-col items-center gap-3">
              <Lock size={24} className="text-[#333333]" />
